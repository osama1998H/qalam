...
import { useFileExplorer, FileNode } from '../stores/useFileExplorer'
...
export default function FileExplorer({ onOpenFile }: FileExplorerProps) {
  ...
  // Restore folder on mount if previously opened
  useEffect(() => {
    if (rootPath && tree.length === 0) {
      loadFolder(rootPath)
    }
  }, [rootPath, tree.length, loadFolder])
  ...
  const handleNewItem = async (type: 'file' | 'folder') => {
    ...
    if (parentPath) {
      setNewItemDialog({ visible: true, type, parentPath })
      setNewItemName('')
    }
    closeContextMenu()
  }
  ...
  const confirmNewItem = async () => {
    ...
    if (newItemDialog.type === 'file') {
      await window.qalam.folder.createFile(fullPath)
    } else {
      await window.qalam.folder.createFolder(fullPath)
    }

    // Refresh the parent folder
    if (newItemDialog.parentPath === rootPath) {
      handleRefresh()
    } else {
      loadChildren(newItemDialog.parentPath)
    }

    setNewItemDialog({ visible: false, type: 'file', parentPath: '' })
  }
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm(`هل تريد حذف "${contextMenu.node.name}"؟`)
    if (confirmed) {
    ...
    <div className="file-explorer" onDragLeave={handleDragLeave}>
      <div className="file-explorer-header">
        ...
        <div className="file-explorer-actions">
          <button
            ...
            onClick={() => {
              if (rootPath) {
                setNewItemDialog({ visible: true, type: 'file', parentPath: rootPath })
                setNewItemName('')
              }
            }}
            title="ملف جديد"
          ...
          <button
            ...
            onClick={() => {
              if (rootPath) {
                setNewItemDialog({ visible: true, type: 'folder', parentPath: rootPath })
                setNewItemName('')
              }
            }}
            title="مجلد جديد"
      ...
      {rootPath ? (
        <div className="file-explorer-content">
          <div className={`file-explorer-root ${isProject ? 'is-project' : ''}`}>
            {isProject ? (
            ...
            <span className="file-explorer-root-name">
              {isProject && projectConfig ? projectConfig.name : rootName}
            </span>
            {isProject && (
      ...
      {contextMenu.visible && (
        ...
        >
          {contextMenu.node?.type === 'directory' && (
            <>
              <button onClick={() => handleNewItem('file')}>
                <Plus size={14} />
                ملف جديد
              </button>
              <button onClick={() => handleNewItem('folder')}>
                <FolderPlus size={14} />
                مجلد جديد
              </button>
      ...
      {newItemDialog.visible && (
        <div className="dialog-overlay" onClick={() => setNewItemDialog({ visible: false, type: 'file', parentPath: '' })}>
          <div className="dialog" onClick={(e) => e.stopPropagation()}>
            <h3>{newItemDialog.type === 'file' ? 'ملف جديد' : 'مجلد جديد'}</h3>
            <input
              type="text"
              value={newItemName}
              onChange={(e) => setNewItemName(e.target.value)}
              placeholder={newItemDialog.type === 'file' ? 'اسم الملف' : 'اسم المجلد'}
              autoFocus
              onKeyDown={(e) => {
                if (e.key === 'Enter') confirmNewItem()
                if (e.key === 'Escape') setNewItemDialog({ visible: false, type: 'file', parentPath: '' })
              }}
            />
            <div className="dialog-actions">
              <button onClick={confirmNewItem} className="primary">إنشاء</button>
              <button onClick={() => setNewItemDialog({ visible: false, type: 'file', parentPath: '' })}>إلغاء</button>
            </div>
          </div>
        </div>
      )}
